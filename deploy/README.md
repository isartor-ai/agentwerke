# Deploying Agentwerke in production
...
| Path | Use when | Where |
| --- | --- | --- |
| **Kubernetes (Helm)** | HA, autoscaling, stronger sandbox isolation | [`helm/agentwerke`](helm/agentwerke) |
...
```bash
# 1. Create the shared secret the chart references (values: secretName=agentwerke-secrets)
kubectl create namespace agentwerke
kubectl -n agentwerke create secret generic agentwerke-secrets \
...
# 2. Install / upgrade
helm upgrade --install agentwerke deploy/helm/agentwerke -n agentwerke \
  --set api.image.tag=<version> --set web.image.tag=<version> --set worker.image.tag=<version> \
  --set api.ingress.enabled=true --set api.ingress.host=agentwerke.example.com
```
...
- **Ingress/TLS:** `api.ingress.enabled`, `.host`, `.tlsSecretName` (`agentwerke-tls`).
...
It pulls pinned published images (`ghcr.io/isartor-ai/agentwerke/{api,web}:${AGENTWERKE_VERSION:-latest}`),
