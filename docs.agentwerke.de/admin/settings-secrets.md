...
Agentwerke has an Admin-only Settings control plane at `/settings` and `/api/settings`. It centralizes the values operators need most often: model provider, integrations, authentication, runtime mode, storage, sandboxing, observability, and Settings storage.
...
Agentwerke starts from normal .NET configuration:
