...
Autofac is self-hostable. A production deployment needs the API, web UI, Postgres, artifact storage, a model provider, integration credentials, and a sandbox strategy for agent execution.
...
Storage__RootPath=/var/lib/autofac/artifacts
Tracing__OtlpEndpoint=http://jaeger:4317
Tracing__ServiceName=autofac
```

## Kubernetes path

Use the Helm chart under `deploy/helm/autofac` for API, dispatch worker, Web, optional Postgres, HPA, RBAC, ingress, TLS, and sandbox-provider wiring. Review `deploy/README.md` before installing into a real cluster.
