...
Autofac loads the configured `Agents:Registry:AgentsDirectory`. If the bundled directory is read-only, set `Agents:Registry:WritableAgentsDirectory` to a writable overlay. Autofac reads the shipped directory first and then the writable overlay.
...
For each agent step, Autofac assembles:
