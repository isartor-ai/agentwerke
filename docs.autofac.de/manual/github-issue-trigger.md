...
Autofac can start a workflow run from a GitHub issue. This lets a team file a feature or bug, opt it into Autofac, and let a governed workflow plan, implement, review, and open a pull request.

## How the trigger works

1. GitHub sends an `issues` webhook to `POST /webhooks/github`.
2. Autofac validates `X-Hub-Signature-256` against `Integrations:GitHub:WebhookSecret`.
3. Autofac checks the issue action against `Integrations:GitHub:TriggerActions`.
4. Autofac checks that the issue has the required label.
5. Autofac resolves the active workflow tagged `github-trigger`.
...
By default, the issue must carry the label `autofac`. This prevents every new issue from spending model budget or opening branches.
...
Integrations__GitHub__BranchPrefix=autofac/
...
1. Add a webhook.
2. Set the payload URL to `https://<your-autofac-host>/webhooks/github`.
...
| Signature validation fails | The GitHub webhook secret and Autofac setting differ. |
