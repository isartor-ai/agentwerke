...
## ADR-002: BPMN-centric Autofac runtime by default

Autofac's default workflow runtime is the in-process, Postgres-backed Autofac runtime. It avoids requiring Camunda for the open-source path and keeps the default deployment simpler.

Operational impact:

- `WorkflowRuntime:Mode=Autofac` is the default.
...
- New quickstart and open-core docs should assume the Autofac runtime unless explicitly stated otherwise.
