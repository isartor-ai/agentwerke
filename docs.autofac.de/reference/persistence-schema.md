...
Autofac uses EF Core migrations in `src/Autofac.Infrastructure` with the default PostgreSQL schema `autofac`.
...
dotnet ef migrations add <Name> \
  --project src/Autofac.Infrastructure \
  --startup-project src/Autofac.Api
```
