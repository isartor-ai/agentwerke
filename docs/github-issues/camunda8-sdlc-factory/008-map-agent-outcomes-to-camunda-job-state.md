...
## Why
Operators need Camunda state and Autofac run state to agree, especially when retries or incidents occur.

## Scope
- Complete job with output variables on success.
- Fail job with retry decrement on retryable failure.
- Surface exhausted retries as incident/blocker in Autofac.
...
- Retryable failures decrement retries and record Autofac events.
...
- Manual failure path shows consistent Camunda and Autofac state.
