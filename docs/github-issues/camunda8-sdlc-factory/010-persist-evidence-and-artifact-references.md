...
## Summary
Store agent outputs, evidence, and artifacts in Autofac, then pass stable references through Camunda variables.

## Why
Camunda variables should not become the artifact store. Autofac needs a durable evidence chain that operators can inspect.

## Scope
- Define evidence produced and evidence required for each agent task.
- Store large outputs in Autofac artifact storage.
