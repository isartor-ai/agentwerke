...
cleanup() {
  if [[ "${AGENTWERKE_AGENT_SANDBOXED_KEEP_STACK:-0}" == "1" ]]; then
    return
  fi
...
wait_for_api() {
  local attempt
  local max_attempts="${AGENTWERKE_AGENT_SANDBOXED_WAIT_ATTEMPTS:-60}"
  local sleep_seconds="${AGENTWERKE_AGENT_SANDBOXED_WAIT_SECONDS:-2}"
