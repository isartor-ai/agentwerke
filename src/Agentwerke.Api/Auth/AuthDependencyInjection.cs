...
{
    public const string PolicyScheme = "AgentwerkeAuth";

    public static IServiceCollection AddAgentwerkeAuth(
        ...
        IWebHostEnvironment environment)
    {
        ...
        {
            o.AddPolicy(AgentwerkePolicies.Viewer, p =>
            ...
            o.AddPolicy(AgentwerkePolicies.Operator, p =>
                p.RequireAuthenticatedUser()
                    .RequireRole(AgentwerkeRoles.Operator, AgentwerkeRoles.Admin));

            o.AddPolicy(AgentwerkePolicies.Approver, p =>
                p.RequireAuthenticatedUser()
                    .RequireRole(AgentwerkeRoles.Approver, AgentwerkeRoles.Admin));

            o.AddPolicy(AgentwerkePolicies.Admin, p =>
                p.RequireAuthenticatedUser()
                    .RequireRole(AgentwerkeRoles.Admin));
        });
    ...
    private static void NormalizeRoleClaims(ClaimsIdentity identity, JwtOptions opts)
    {
        ...
        foreach (var role in AgentwerkeRoleMapper.ResolveRoles(roleClaimValues, opts))
        {
    ...
    private static void EnrichRolesFromLdap(ClaimsIdentity identity, JwtOptions opts, IServiceProvider services)
    {
        ...
        foreach (var role in AgentwerkeRoleMapper.ResolveRoles(groups, opts))
        {
            if (existingRoles.Add(role))
            {
                identity.AddClaim(new Claim(ClaimTypes.Role, role));
