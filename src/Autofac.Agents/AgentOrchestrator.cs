...
public sealed class AgentOrchestrator : IServiceTaskExecutor
{
        ...
        if (IsGitHubPullRequestAction(metadata.Action))
        {
            ...
            return CreateToolRequest(
                ...
                new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
                {
                    ...
                    ["body"] = BuildPullRequestBody(runId, stepId, metadata, attempt, runContext),
        ...
        IReadOnlyDictionary<string, string> runContext)
    {
        ...
        // #150: when opted in, include the prior agent step output(s) so the PR (its
        // committed marker file + body) carries the agent's work, not just metadata.
        if (metadata.IncludeAgentOutput)
        {
            var agentOutput = CollectAgentOutput(metadata.OutputFrom, runContext);
    ...
    /// <summary>
    /// Gathers agent output from run context for the PR body (#150). With an explicit
    /// <paramref name="outputFrom"/> node id, returns <c>output.&lt;id&gt;</c>; otherwise
    /// concatenates all <c>output.*</c> entries under per-node headings (ordered by key).
    /// </summary>
    internal static string CollectAgentOutput(string? outputFrom, IReadOnlyDictionary<string, string> runContext)
    {
        if (!string.IsNullOrWhiteSpace(outputFrom))
        {
            return runContext.TryGetValue($"output.{outputFrom}", out var single) ? single : string.Empty;
        }

        var outputs = runContext
            .Where(static kv => kv.Key.StartsWith("output.", StringComparison.OrdinalIgnoreCase)
                && !string.IsNullOrWhiteSpace(kv.Value))
            .OrderBy(static kv => kv.Key, StringComparer.OrdinalIgnoreCase)
            .Select(static kv => $"### {kv.Key["output.".Length..]}\n\n{kv.Value}");
