...
/// constructed directly by <c>Autofac.AgentRunner.RunnerToolFactory</c>, never registered in
/// Autofac.Agents' DI container, so the in-process (non-sandboxed) orchestrator never sees them.
/// </summary>
    ...
    string apiHost = "github.com") : IAgentTool, IToolSchemaProvider
{
    ...
    {
        await SandboxWorkspace.RunProcessAsync("git", ["config", "user.email", "autofac-bot@autofac.de"], workspaceRoot, cancellationToken);
