...
public sealed class AgentSandboxedFactAttribute : FactAttribute
{
    ...
    {
        var enabled = Environment.GetEnvironmentVariable("AGENTWERKE_AGENT_SANDBOXED_E2E");
        if (!string.Equals(enabled, "true", StringComparison.OrdinalIgnoreCase)
            && !string.Equals(enabled, "1", StringComparison.OrdinalIgnoreCase))
        {
            Skip = "Skipped: set AGENTWERKE_AGENT_SANDBOXED_E2E=true to run agent_sandboxed workflow E2E tests.";
        }
    }
}
